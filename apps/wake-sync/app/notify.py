    ...
    for key in fields:
        ...
        if key == "smtp_port":
            try:
                ...
                val = 587
        if key in ("notify_on_fail", "notify_on_success", "telegram_enabled", "email_enabled", "smtp_tls"):
            val = bool(val)
        cur[key] = val
    save_settings(cur)
...
def notify_event(
    ...
    *,
    ...
    lng = lang or get_lang()
    settings = get_notify_settings()
    if success and not settings.get("notify_on_success"):
        return
    if not success and not settings.get("notify_on_fail"):
        return

    if success:
        subject = t("notify.ok_subject", lng, name=plan_name)
        body = t("notify.ok_body", lng, name=plan_name, route=route, message=message)
    else:
        subject = t("notify.fail_subject", lng, name=plan_name)
        body = t("notify.fail_body", lng, name=plan_name, route=route, message=message)
