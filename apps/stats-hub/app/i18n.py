...
STRINGS: dict[str, dict[str, str]] = {
    "de": {
        ...
        "ugos.hint": "Port = UGOS-API-Port aus den NAS-Einstellungen (abweichende Ports sind OK). HTTPS muss zum Port passen. Host meist 127.0.0.1. Stats-Hub-Oberfläche: Port 29125.",
        "ugos.https_hint": "HTTPS an = API per TLS (z. B. Port 9443 oder euer HTTPS-Port). HTTPS aus = HTTP (z. B. Port 9999 oder euer HTTP-Port).",
        "ugos.test": "Testen",
        "ugos.test_ok": "Verbindung OK",
        "ugos.test_fail": "Fehler: {msg}",
    ...
    "en": {
        ...
        "ugos.title": "UGOS Web API",
        "ugos.hint": "Port = UGOS API port from NAS settings (custom ports are fine). HTTPS must match the port. Host usually 127.0.0.1. Stats Hub UI: port 29125.",
        "ugos.https_hint": "HTTPS on = TLS API (e.g. port 9443 or your HTTPS port). HTTPS off = HTTP (e.g. port 9999 or your HTTP port).",
        "ugos.test": "Test",
        "ugos.test_ok": "Connection OK",
        "ugos.test_fail": "Error: {msg}",
