...
STRINGS: dict[str, dict[str, str]] = {
    "de": {
        ...
        "profile.smb_performance": "SMB Performance-Modus (schneller, bei Fehlern automatisch stabil)",
        "profile.smb_performance_hint": "Aktiviert rclone Multi-Thread/SMB-Chunk-Writes. Kann auf manchen Windows-PCs scheitern — dann Fallback auf stabilen Modus (~50 % Gigabit).",
        ...
        "profiles.mode_perf": "SMB Performance",
        ...
        "benchmark.title": "Geschwindigkeitstest",
        "benchmark.hint": "Misst SMB-Lesegeschwindigkeit (PC-Freigabe → NAS) und optional NAS-Volume Schreib/Lese. Testdatei wird automatisch angelegt und wieder entfernt.",
        "benchmark.smb_run": "SMB-Test starten",
        "benchmark.disk_run": "Volume-Test starten",
        "benchmark.device": "SMB-Gerät",
        "benchmark.share_ph": "Freigabe (z. B. D) — Pflicht",
        "benchmark.volume": "Ziel-Volume (NAS)",
        "benchmark.size": "Testgröße (MB)",
        "benchmark.running": "Test läuft — Fortschritt unten und im Log …",
        "benchmark.started": "Benchmark gestartet",
        "benchmark.already_running": "Ein Benchmark läuft bereits",
        "benchmark.not_found": "Benchmark nicht gefunden",
        "benchmark.timeout": "Benchmark-Timeout — Details im Log",
        "benchmark.phase_prepare": "Vorbereitung …",
        "benchmark.phase_connect": "Verbindung prüfen …",
        "benchmark.phase_shares": "Freigaben lesen …",
        "benchmark.phase_seed": "Testdatei hochladen …",
        "benchmark.phase_stable": "Stabiler Modus (Lesen) …",
        "benchmark.phase_performance": "Performance-Modus (Lesen) …",
        "benchmark.phase_write": "Schreiben …",
        "benchmark.phase_read": "Lesen …",
        "benchmark.phase_cleanup": "Aufräumen …",
        "benchmark.phase_done": "Fertig",
        "benchmark.result_smb": "SMB {host} / {share} → {vol}: stabil {stable} MiB/s, Performance {perf} MiB/s (Gigabit ≈ {gigabit} MiB/s)",
        "benchmark.result_disk": "{vol}: Schreiben {write} MiB/s, Lesen {read} MiB/s",
        "benchmark.failed": "Benchmark fehlgeschlagen",
        "benchmark.err_device": "Bitte SMB-Gerät wählen",
        "benchmark.err_share": "Bitte SMB-Freigabe eintragen (z. B. D)",
        "benchmark.err_admin_share": "Windows-Admin-Freigaben (z. B. ADMIN$, C$) sind nicht erlaubt — normale Freigabe wählen",
        "benchmark.err_seed": "Testdatei konnte nicht auf die Freigabe geschrieben werden — Schreibrecht prüfen",
        "benchmark.no_smb_device": "Kein gespeichertes SMB-Gerät — zuerst PC hinzufügen.",
    },
    "en": {
        "app.title": "Transfer Hub",
        ...
        "profile.smb_performance": "SMB performance mode (faster, auto-fallback to stable on errors)",
        "profile.smb_performance_hint": "Enables rclone multi-thread/SMB chunk writes. May fail on some Windows PCs — then falls back to stable mode (~50% of gigabit).",
        ...
        "benchmark.title": "Speed test",
        "benchmark.hint": "Measures SMB read speed (PC share → NAS) and optional NAS volume write/read. Test file is created and removed automatically.",
        "benchmark.smb_run": "Run SMB test",
        "benchmark.disk_run": "Run volume test",
        "benchmark.device": "SMB device",
        "benchmark.share_ph": "Share (e.g. D) — required",
        "benchmark.volume": "Target volume (NAS)",
        "benchmark.size": "Test size (MB)",
        "benchmark.running": "Test running — see progress below and in log …",
        "benchmark.started": "Benchmark started",
        "benchmark.already_running": "A benchmark is already running",
        "benchmark.not_found": "Benchmark not found",
        "benchmark.timeout": "Benchmark timeout — see log for details",
        "benchmark.phase_prepare": "Preparing …",
        "benchmark.phase_connect": "Checking connection …",
        "benchmark.phase_shares": "Listing shares …",
        "benchmark.phase_seed": "Uploading test file …",
        "benchmark.phase_stable": "Stable mode (read) …",
        "benchmark.phase_performance": "Performance mode (read) …",
        "benchmark.phase_write": "Writing …",
        "benchmark.phase_read": "Reading …",
        "benchmark.phase_cleanup": "Cleaning up …",
        "benchmark.phase_done": "Done",
        "benchmark.result_smb": "SMB {host} / {share} → {vol}: stable {stable} MiB/s, performance {perf} MiB/s (gigabit ≈ {gigabit} MiB/s)",
        "benchmark.result_disk": "{vol}: write {write} MiB/s, read {read} MiB/s",
        "benchmark.failed": "Benchmark failed",
        "benchmark.err_device": "Please select an SMB device",
        "benchmark.err_share": "Please enter an SMB share (e.g. D)",
        "benchmark.err_admin_share": "Windows admin shares (e.g. ADMIN$, C$) are not allowed — use a normal share",
        "benchmark.err_seed": "Could not write test file to share — check write permission",
        "benchmark.no_smb_device": "No saved SMB device — add a PC first.",
    },
}
